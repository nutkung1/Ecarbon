...
class Fuel_Tab:
    ...
    def fuel_create(self):
        ...
        if create_button:
            self.mycursor.execute("SELECT COUNT(*) FROM fuel")
            result = self.mycursor.fetchone()[0]
            sql = "insert into fuel(fuel_id, cultivated_areas_id, fuel_used_in_kilogram) values(%s,%s,%s)"
            val = (result+1, cultivated_area_id, fuel_used)
    ...
    def fuel_update(self):
        ...
        if update_button:
            sql = "UPDATE fuel set cultivated_areas_id=%s, fuel_used_in_kilogram=%s WHERE fuel_id=%s"
    ...
    def fuel_delete(self):
        ...
        if delete_button:
            sql = "DELETE FROM fuel WHERE fuel_id=%s"
            val = (id,)
            self.mycursor.execute(sql, val)
            self.mydb.commit()
