...
class carbon_offset:
    ...
    def carbon_offset_create(self):
        ...
        if create_button:
            ...
            sql = "insert into carbon_offset(carbon_offset_id, cultivated_areas_id, total_carbon_offset, filer_cake_in_rai, beans_in_rai) values(%s,%s,%s,%s,%s)"
            val = (result+1,cultivated_area_id,total_carbon_offset,filter_cake,beans_rai)
    ...
    def carbon_offset_update(self):
        ...
        if update_button:
            sql = "UPDATE carbon_offset set cultivated_areas_id=%s, total_carbon_offset=%s, filer_cake_in_rai=%s, beans_in_rai=%s WHERE carbon_offset_id=%s"
            val = (cultivated_area_id, total_carbon_offset, filter_cake, beans_rai, id)
            self.mycursor.execute(sql, val)
            self.mydb.commit()
